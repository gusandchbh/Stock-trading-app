...
const SalePage = () => {
  ...
  const handleSale = async () => {
    ...
    try {
      ...
      const response = await API.post(
        `/api/v1/users/${userToken.userId}/stocks/${marketStockId}/sell?quantity=${quantity}`
      );
  ...
  return (
    <Container
      fluid
      className="vh-100 d-flex justify-content-center align-items-start bg-dark pt-5"
    >
      <Card
        className="text-center bg-dark border-0 shadow"
        style={{ maxWidth: "500px", color: "#fff", backgroundColor: "#333" }}
      >
        <Card.Body>

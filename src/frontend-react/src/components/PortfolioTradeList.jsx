...
const PortfolioTradeList = ({ portfolioId }) => {
  ...
  const fetchTrades = (page = 0, searchTerm = "") => {
    return API.get(
      `/api/v1/trade/${portfolioId}?searchTerm=${searchTerm}&page=${page}&size=10`
    );
  };
  ...
  const {
    ...
    ["trades", currentPage, searchTerm],
    () => fetchTrades(currentPage - 1, searchTerm),
    {
      staleTime: 5000,
    }
  );
      ...
      {isLoading ? (
        ...
        <>
          ...
          <Pagination className="justify-content-center my-4">
            ...
            <Pagination.Next
              disabled={trades && trades.data.totalPages <= currentPage}
              onClick={() => setCurrentPage(currentPage + 1)}
            />
          </Pagination>
        </>

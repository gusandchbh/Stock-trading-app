...
public class PortfolioStockService {
  ...
  @Autowired
  public PortfolioStockService(MarketStockRepository marketStockRepository,
  ...
  public MarketStock findStockById(Long stockId) {
    return marketStockRepository.findById(stockId)
  ...
  public String purchaseStock(Long userId, Long stockId, int quantity) {
    ...
    PortfolioStock portfolioStock =
        updatePortfolioAndPurchaseTrade(portfolio, marketStock, quantity, purchasePricePerShare,
            totalPrice);
  ...
  private PortfolioStock updatePortfolioAndPurchaseTrade(Portfolio portfolio,
                                                         MarketStock marketStock, int quantity,
    ...
    Trade trade =
        createTrade(portfolio, marketStock, quantity, purchasePricePerShare, totalPrice,
            TradeType.BUY);
  ...
  public String sellStock(Long userId, Long stockId, int quantity) {
    ...
    updatePortfolioAndTrade(portfolio, marketStock, quantity, sellPricePerShare, amount,
        TradeType.SELL);
  ...
  private void checkSufficientShares(int quantity, PortfolioStock portfolioStock) {
    if (quantity > portfolioStock.getQuantity()) {
      throw new PortfolioStock.InsufficientSharesException(
          "Insufficient quantity to sell the stock");
    }
  }
  ...
  private void updatePortfolioAndTrade(Portfolio portfolio, MarketStock marketStock, int quantity,
                                       BigDecimal sellPricePerShare, BigDecimal amount,
                                       TradeType tradeType) {
    portfolioService.updateAccountBalance(portfolio, amount);

    Trade trade =
        createTrade(portfolio, marketStock, quantity, sellPricePerShare, amount, tradeType);
    tradeRepository.save(trade);

    updatePortfolioStock(portfolio, marketStock, quantity);
  }

  private Trade createTrade(Portfolio portfolio, MarketStock marketStock, int quantity,
